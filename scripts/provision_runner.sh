...
EOF
  ...
  aws:network)
    ...
    RUNNER_ROUTE_TABLE_ID="$(tofu_output_raw "$tofu" "$BASIC_TOFU_DIR" runner_route_table_id)"
    ...
    SERVER_IMAGE_ID="$(tofu_output_raw "$tofu" "$BASIC_TOFU_DIR" network_server_image_id)"
    cat >"$tmp_tfvars" <<EOF
...
existing_runner_route_table_id = "${RUNNER_ROUTE_TABLE_ID}"
runner_subnet_cidr = "${RUNNER_SUBNET_CIDR}"
ssh_ingress_cidr = "${REMOTE_SSH_INGRESS_CIDR}"
assign_public_ip = false
instance_affinity = "none"
root_volume_size_gib = 30
