...
usage: $0 --action probe|start|stop [--out PATH] [--pid-file PATH] [--snapshot PATH]
          [--deep-latency-us N] [--target-us N] [--device PATH]
...
CPUIDLE_ROOT=/sys/devices/system/cpu/cpuidle
CPU0_STATES=/sys/devices/system/cpu/cpu0/cpuidle
  ...
  stop)
    ...
    if [[ -f "$SNAPSHOT" ]]; then
      ...
      if [[ "$before" =~ ^[0-9]+$ && "$after" =~ ^[0-9]+$ ]]; then
        delta=$((after - before))
        # The bound held if essentially nothing entered a deep state while it
        # was in force. A non-zero delta means the request did not stick, which
        # is exactly the case that must not be mistaken for a pinned run.
